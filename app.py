...
def quantize_colors(rgb_img: Image.Image, n_colors: int):
    """
    Quantize RGB image to at most n_colors. Returns (label_map H×W, palette list).

    PIL's quantize() may return fewer palette entries than n_colors when:
      - The image has fewer unique colours than n_colors
      - The PIL version caps the palette at actual unique colours
    Using range(n_colors) on a shorter palette causes IndexError.
    Fix: derive actual_colors from len(getpalette())//3, not from n_colors.
    Also clamp label_map values so no index can ever exceed actual_colors-1.
    """
    quant      = rgb_img.quantize(colors=n_colors, dither=0)
    raw        = quant.getpalette()               # flat [R,G,B, R,G,B, ...]
    actual     = len(raw) // 3                    # real number of palette entries
    if actual == 0:
        raise RuntimeError("quantize() returned an empty palette.")
    palette    = [(raw[i*3], raw[i*3+1], raw[i*3+2]) for i in range(actual)]
    # Clamp label indices so they never exceed the palette we actually have
    label_map  = np.clip(np.array(quant, dtype=np.int32), 0, actual - 1)
